...
def run_demo():
    ...
    atm.init(
        data_manager="sqlite",
        db_path="usage.db",
        default_app_id="finance-advisor"
    )
    ...
    def auditor_pre_hook(event):
        print(f"   [Pre-hook] Auditing event. Type: {event.event_type}, Tokens: {event.token_count}")
        if event._additional_metadata_config:
            print(f"             Custom Config metadata: {event._additional_metadata_config}")
    ...
    with atm.context(username="free-user", app_id="basic-portal", department="marketing"):
        print("First LLM call (40 tokens)...")
        atm.add_user_request("Tell me a short joke.", token_count=40)
        
        print("Second LLM call (40 tokens)...")
        atm.add_user_request("Tell me another short joke.", token_count=40)
        
        print("Third LLM call (30 tokens - Should breach minute limit of 100)...")
        try:
            # 40 + 40 + 30 = 110 > 100
            atm.add_user_request("Tell me a third joke.", token_count=30)
