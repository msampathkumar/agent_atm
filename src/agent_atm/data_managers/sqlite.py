...
# ==============================================================================
# DEPRECATED in v1.0: Raw SqliteManager implementation has been deprecated.
# Rationale: Standardizing exclusively on SQLAlchemyManager eliminates schema drift,
# prevents duplicate persistence logic, and supports cross-dialect scalability.
# SqliteManager now acts as a direct proxy subclass to SQLAlchemyManager for backwards compatibility.
# ==============================================================================
...
class SqliteManager(SQLAlchemyManager):
    """Persistent SQLite DataManager implementation (Deprecated in v1.0).
    
    DEPRECATED in v1.0: Standardized on SQLAlchemyManager to prevent schema drift
    and eliminate redundant persistence logic. SqliteManager now acts as a direct proxy
    to SQLAlchemyManager for backwards compatibility.
    """
    def __init__(self, db_path: str = "agent_atm.db"):
        warnings.warn(
            "SqliteManager is deprecated in v1.0 and will be removed in a future release. "
            "Please use SQLAlchemyManager or initialize via atm.init(data_manager='sqlite').",
            DeprecationWarning,
            stacklevel=2
        )
        self.db_path = db_path
        super().__init__(db_url=db_path)
